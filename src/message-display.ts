...
export class MessageTarget {
  constructor(
    ...
    private config: Required<MessageTargetConfig>
  ) {}
...
export class MessageDisplay {
  ...
  for(elementId: string, config: MessageTargetConfig = {}): MessageTarget {
    ...
    const defaultConfig: Required<MessageTargetConfig> = {
  ...
  private processMessage(message: string, type: StatusType): string {
    return match(type)
  ...
  private logToConsole(message: string, type: StatusType, elementId?: string): void {
    ...
    setTimeout(() => {
      ...
      const animate = () => {
        ...
        statusElement.style.setProperty(
          "transform",
          `rotate(${tilt}deg) translate(${currentX}px, ${currentY}px)`,
          "important"
        );
